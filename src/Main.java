...
public class Main {
    public static void main(String[] args) {
        ...
        int pocet = 0,operace = 0, pozice1 = 0, pozice2 = 0;
        for (int x = 0; x <= PoleCisel.length; x++) {
            pocet++;
            if (pocet == 1) {
                if (PoleCisel[x] == 99)
                    break;
                ...
                if (operace == 1) {
                    PoleCisel[PoleCisel[x]] = PoleCisel[pozice1] + PoleCisel[pozice2];
                } else if (operace == 2) {
                    PoleCisel[PoleCisel[x]] = PoleCisel[pozice1] * PoleCisel[pozice2];
                } else System.out.println("something went wrong");
                pocet = 0;
            } else System.out.println("Cislo nebylo rozpoznáno");
        }
